...
**禁止「受限读取 + 整文件写回」组合**：不得用小于文件总行数的 `limit` 读取后把结果整文件 `write` 回去——会静默截断文件（历史事故：`04-TODO.md` 两次被截，恢复自 git HEAD）。

- 更新文本/文档文件优先使用 `edit` 做**定点替换**（old_string 唯一、不动未涉及行）。
- 确需整文件 `write` 时，先 `read`（**不设 `limit`**，默认覆盖全部行）并核对 `lines.length === totalLines`，确认读取完整才允许写回。
- `read` 只用于查看头部/定位时，可传 `offset`/`grep`/小 `limit`，但**不得**把其结果整文件写回。
- 写后自检：`git diff` 应只显示目标改动行；文档若含 `P3`/`§文档`/`§已归档` 等段落，写后抽查其仍存在。
