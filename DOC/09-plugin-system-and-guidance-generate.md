...
> **正则（2026-06-08）**：**不**经 capabilities；已定为宿主原生 **`host.regex`**（**`DOC/24`** §2）。下文 `regex-transform` 示例**废止**，仅作能力模型参考。
...
- [x] 正则原生 + `host.regex`（**`DOC/24`** · 2026-06-10）；`conversation-export` 消费导出勾选规则
...
> **详案**：[`DOC/38-plugin-sandbox-and-host-evolution.md`](38-plugin-sandbox-and-host-evolution.md)
...
| 二次 LLM 拼 prompt | **`DOC/39`** · `resolveContextBlocks` + `assemblePluginPrompt`；泛化 Historian `prepareContext` |
| 沙箱 | Worker + Host API 代理；当前同进程 `import` 无隔离 |
| 性能 | 描述符改造后再沙箱；整包 messages IPC 粗估 +5～50 ms |
| 分期 | Phase A 注入 → Phase B Worker → Phase C apiConfigId 白名单 |
