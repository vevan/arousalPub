...
| | `prepareContextBlocks(req)` | ✅ | `POST …/prepare-context`（`blocks[]` · **`DOC/39`**） |
| | `assemblePluginPrompt(req)` | ✅ | `POST …/assemble-plugin-prompt`（**`DOC/39`**） |
| | `completeWithContext(req)` | ✅ | `POST …/complete-with-context`（**`DOC/39`** · 主入口） |
