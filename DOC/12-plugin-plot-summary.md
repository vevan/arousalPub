...
- **布局**：章回 divider **下方**独立一行（`ChatTurnBlock` · `turn-block-head` slot）；宿主对该行**常驻** `min-height` 占位，避免插件图标懒加载后滚动位置跳动。
- **按钮顺序**：**左 ▷ 起始**、**右 ◁ 结束**；同插件多按钮按 **`registerSlotButton` 注册先后**（宿主 `order` 字段），**非** `id` 字母序。
- **样式**：MDI 圆环图标自带边框；宿主 `.plugin-slot` **不**再描边；选中态用 `cm-range-start--active` / `cm-range-end--ready` 配色。
