...
| `{{notChar}}` | ✅ | 非群聊：除首绑卡外；群聊：除当前 speaker（`DOC/35` §5） |
...
| `{{original}}` `{{notChar}}`（群聊语义） | ✅ | 群聊下 `notChar` = 除当前 speaker；见 `DOC/35` |
| `{{isMobile}}` | ⏳ | 可选 |
...
| ST 宏 / 能力 | 标记 | 原因 |
|--------------|------|------|
| `{{group}}` `{{groupNotMuted}}` `{{charIfNotGroup}}` | ✅ | 群聊定案见 [`DOC/35`](35-group-chat.md) |
