...
- 分支子目录 `branch*/` 与 `meta.links.branches` 的产品 UI 与写入路径 — **完整参考见 `DOC/devNotes/23-conversation-branches.md`**（memory 枚举 / 召回过滤等服务端原语已落地；**创建定案**：空分支 + 从下一轮继续 · `DOC/devNotes/23` §1.4）
- `GET .../messages` 分页 / 按 chunk 懒加载 — **`DOC/devNotes/15-conversation-messages-lazy-load.md`（✅）**
...
**分支子目录**：与主路径**同一公式**；可与父目录 basename 相同但路径不同（`branch1/turn-000100-000199.json` ≠ 根目录同名文件）。fork 在 160 且分支首条 turn 为 161 时，分支首块为 `turn-000100-000199.json`，**不是**因「新建分支」而默认 `turn-000000-000099.json`。见 **`DOC/devNotes/23` §1.4–§1.5**。
...
- **当前实现（2026-07-08）**：**`chat-audit.json`**（`schemaVersion: 3`）；**不再**读盘 **`chat-prompt.json`**。滚动 tail 后新轮用**新块文件名**，无需迁移旧 audit 条目。
- 定案与验收：**`DOC/devNotes/24` §3**、**`DOC/devNotes/03`** 审计段落。
...
| **S7** | 文档：`DOC/devNotes/03` §6 增「实现状态」、`DOC/devNotes/06` 摘要 | — |
...
## 9. 与 `DOC/devNotes/04` 的对应

- [x] `DOC/devNotes/04-TODO.md` → **「Chunk 链按轮数切分与全链读取」**
- [x] `DOC/devNotes/03` §6 实现状态指向本文
