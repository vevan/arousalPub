...
> **读者**：**第三方插件作者请优先阅读 [`DOC/devNotes/18-plugin-host-developer-api.md`](18-plugin-host-developer-api.md)**（宿主 API 单一入口）；本文保留产品与边界定案。  
> **关联**：`DOC/devNotes/03` §1.3、`DOC/devNotes/09` §5、`DOC/devNotes/10`、`DOC/devNotes/12`（Historian（剧情纪要）插件示例）。
...
### 0.1 已有（今日即可用，见 `DOC/devNotes/10` / `DOC/devNotes/09`）
...
| 注册 | `registerSlotButton` / `registerFormDialog` / `registerStyles` | 见 `DOC/devNotes/09` §8、`DOC/devNotes/18` §3.1 |

### 0.2 扩展 API（`DOC/devNotes/11` 规划项 + 2026-06-02 落地）
...
| | `applyOrder(lorebookId, req)` | ✅ | `POST …/lorebooks/:id/apply-order`，按 layout 写 `group.order` / `entry.order`（`DOC/devNotes/12` §4.2–§4.3） |
| | `ensure(req?)` | ✅ | `POST …/lorebooks/ensure`，自动建 summary 书，见 `DOC/devNotes/12` §2.3 |
| **`host.api`** | `listPresets()` | ✅ | `GET /api/settings` |
| **`host.plugin`** | `complete(req)` | ✅ | `POST …/complete` |
| | `prepareContextBlocks(req)` | ✅ | `POST …/prepare-context`（`blocks[]` · **`DOC/devNotes/39`**） |
| | `assemblePluginPrompt(req)` | ✅ | `POST …/assemble-plugin-prompt`（**`DOC/devNotes/39`**） |
| | `completeWithContext(req)` | ✅ | `POST …/complete-with-context`（**`DOC/devNotes/39`** · 主入口） |
...
| 项 | 定案 |
|----|------|
| 摘要 / Tracker | **插件业务**；宿主**不**提供 `summarize`、不内置摘要 prompt；**迹录（Trace Keeper）**定案见 **`DOC/devNotes/30`** |
...
- 对话级 **`index.json` → `pluginSettings[pluginId].apiConfigId`**（插件 Tab · `type: apiPreset`）表示本对话对该插件的 API 覆盖（见 `DOC/devNotes/03` §1.2.1）。  
...
**非目标**：宿主 **不** 实现 Historian 的 other → sidecar → summary 排序规则；示例见 `DOC/devNotes/12` §4.2（`computePlotSummaryApplyOrderLayout`）。
...
**多书绑定 XML（已实现，见 `DOC/devNotes/03` §13.2）**：`lorebookIds.length > 1` 时，命中条目按 **资料库 `name`** 分组为 `<lorebook name="…">` 子块；单本仍用扁平 `<lores><lore>…</lore></lores>` 以兼容旧 prompt。

**插件新建条目**：`createEntry` 的 `triggerMode` / `keys` 由插件 settings 默认（Historian（剧情纪要）见 `DOC/devNotes/12` `defaultEntryTriggerMode`）；与注入扫描规则一致。
...
- 插件 **不得** 直接改 `apiPreset` 或其它功能的连接设定（`DOC/devNotes/03` §1.3）。
...
- 插件私有 `settings.json` 内 `progressByConversation`（`DOC/devNotes/10` §5 模式），会话字段优先。

实现时二选一并写入插件文档。

---

## 5. 与 `DOC/devNotes/10` 的关系

| 场景 | 用哪个文档 |
|------|------------|
| 读改对话 turn | **`DOC/devNotes/10`** — `host.conversation.runScope` |
| 资料库 / 补全 / 会话 pluginSettings | **本文 §0.2** |
| Slot / lifecycle / 性能约定 | **`DOC/devNotes/09`** §8 |
...
- [x] `host.conversation.getPluginSettings` / `patchPluginSettings`（含 Pinia snapshot · `DOC/devNotes/21`）
- [x] `host.plugins.getUserSettings` 缓存 + snapshot + 订阅（`DOC/devNotes/32` · `a7ca4ea`）
