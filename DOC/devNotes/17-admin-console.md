...
> **优先级**：P1（见 `DOC/devNotes/04-TODO.md`）  
> **定案（用户确认）**：**仅本机可访问**；**初始用户 `00000000`（种子账号）即管理员**，不引入 `role` 字段。  
> **关联**：`server/src/short-id.ts`（`RESERVED_USER_ID`）、`users-index.ts`、`DOC/devNotes/25` §15.2 密钥轮换。
...
| 密钥轮换 | 手动触发 `DOC/devNotes/25` §15.2 DEK 重加密（维护模式 + 进度） |
...
已实现（**`DOC/devNotes/25-security-deployment.md`** §4.2）：
