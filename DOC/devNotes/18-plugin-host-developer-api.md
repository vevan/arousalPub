...
> **宿主实现强制约束**：[`DOC/devNotes/41-plugin-host-generic-principles.md`](41-plugin-host-generic-principles.md) · `.cursor/rules/plugin-host-generic.mdc` — **用户对宿主特化深恶痛绝；绝不允许任何一个功能特化相关字节出现在宿主中**（含能力特化，不限 id 字面量）。**本文档**面向插件作者，下文 bundled 插件名仅作**说明性示例**。

| 项 | 定案 |
|----|------|
| **宿主通用性** | 宿主只提供 generic 能力；**零字节特化**；插件差异由 manifest + hook/action 表达 — 详见 **DOC/devNotes/41 §0–§7** |
...
| `prepareContextBlocks(req)` | **步骤 1** 取块：`ContextBlockSpec[]` → blocks / entriesByBlock；**`DOC/devNotes/39` §3.1** |
| `assemblePluginPrompt(req)` | **步骤 2** blocks + layout → `messages[]`；须 `anchorToTurn`；**`DOC/devNotes/39` §3.2** |
| `completeWithContext(req)` | **主入口**：resolve API → 步骤 1+2 → complete；可选 `draft` 解析；**`DOC/devNotes/39` §3.3** |
...
**`prepareContextBlocks` 请求**（`conversationId` 由宿主注入 · 步骤 1 · **`DOC/devNotes/39` §3.1**）：
...
**`assemblePluginPrompt` 请求**（步骤 2 · 强制独立 RPC · **`DOC/devNotes/39` §3.2**）：
...
**`completeWithContext` 请求**（主入口 · **`DOC/devNotes/39` §3.3**）：
...
| 方法 | 说明 |
|------|------|
| `getUserSettings()` | 读合并后的全局插件 settings（schema 默认值 + 用户文件）。**已加载**则返回 Pinia snapshot（未加载则 `GET` 后写入 cache），与会话 `getPluginSettings` 一致（`DOC/devNotes/32`） |
| `getUserSettingsSnapshot()` | 同步读 global settings snapshot；未加载返回 `{}`（`DOC/devNotes/32`） |
| `onUserSettingsChanged(handler)` | 系统设置 → 插件页保存 global settings 后回调；notify 携带已保存 payload，订阅方无需二次 GET（`DOC/devNotes/32`） |
...
| 方法 | 说明 |
|------|------|
| `notify(title, body?, opts?)` | **唯一通知出口**（[`DOC/devNotes/40`](40-notification-center.md)）：经 store 入队浮层 + 立即写入列表；`snackbar` 默认 true |
...
#### 3.13.1 `host.ui.panel`（✅ · **`DOC/devNotes/30`** 迹录）
...
### 3.14 `host.regex`（✅ · **`DOC/devNotes/24`** §2 · 2026-06-10）
...
**消费示例**：`conversation-export` 导出前 `applyText`（`display`）；`plot-summary` 清理 sidecar 等（见 **`DOC/devNotes/24`** §2.6）。
...
| Hook | 时机 | 典型用途 |
|------|------|----------|
| `resolveAfterAssemblePromptsAddition(ctx, api)` | `/api/chat` 组装 messages 之后（推荐） | 返回注入描述符（规划：`chat` depth + order）；宿主 post-user 区归并 + token 预算。定案见 **`DOC/devNotes/38`** §3 |
...
- 插件 **`runPluginAction(action, body, api)`** 返回：
  - 成功：`{ ok: true, …自定义字段 }`；可选 **`turnMerge`**：`{ turnOrdinal, receiveId, assistantContent, entry }` 由宿主按 **`receiveId` 定位 segment** 合并 `segments[i].receives` 正文与 `turn.plugins[]`（见 **`DOC/devNotes/44`**）。
...
| POST | `/api/plugins/:id/settings/import` | 登录即可（整表替换 settings + 写 `enabled`；见 `DOC/devNotes/09` §4） |
...
- 用于入库与召回前从 turn 语料中移除插件块，避免向量被大块 JSON 淹没；详见 **`DOC/devNotes/03` §14.4.4**。
...
- manifest 可选 **`conversationSettingsSchema`**（字段类型与 §6.1 相同）；完整定案见 **`DOC/devNotes/21`**。
...
- 自动把摘要 lorebook 绑进对话 `lorebookIds`（注入由用户在对话设置勾选，见 `DOC/devNotes/12` §1.1）。
...
| 能力 | 说明 |
|------|------|
| **组装注入描述符 + post-user 归并** | Phase A · **`DOC/devNotes/38`** §3 · guidance / trace-keeper order 定案 |
| **服务端插件 Worker 沙箱** | Phase B · Host API 代理 · **`DOC/devNotes/38`** §2、§4 · `PLUGIN_SERVER_SANDBOX` |
| **插件上下文块 + Prompt 组装** | **`DOC/devNotes/39`** · 扩展 `prepareContext` · `assemblePluginPrompt` · `completeWithContext` |
| **通知中心** | **`DOC/devNotes/40`** · **localStorage** 存储/已读/列表 · `host.ui.notify` 迁入 |
...
| 文档 | 内容 |
|------|------|
| `DOC/devNotes/09-plugin-system-and-guidance-generate.md` | 插件系统、manifest、懒加载、设置页 |
| `DOC/devNotes/10-plugin-conversation-host.md` | 对话 DTO、runScope 细节、swipe/export |
| `DOC/devNotes/11-plugin-host-completion-and-lorebook.md` | 补全与 lorebook 产品设计定案 |
| `DOC/devNotes/12-plugin-plot-summary.md` | Historian（剧情纪要）完整业务示例 |
| `DOC/devNotes/24-regex-and-session-audit.md` | 宿主原生正则三阶段、`host.regex` / `api.regex` |
| `DOC/devNotes/30-plugin-trace-keeper.md` | **迹录** Trace Keeper（✅ v1） |
| `DOC/devNotes/38-plugin-sandbox-and-host-evolution.md` | 插件沙箱、注入描述符、API 绑定（宿主选择器） |
| `DOC/devNotes/39-plugin-context-and-prompt-assembly.md` | 二次 LLM 上下文块 + prompt 组装（**Phase 1–2 已落地**） |
...
| 日期 | 说明 |
|------|------|
| 2026-06-02 | 首版：合并 DOC/devNotes/10–11 已实现 API；新增 `prepareContext`、`normalizeEntryRefs`、`completeDraft` |
...
| 2026-07-07 | **DOC/devNotes/39 落地**：`prepareContextBlocks` / `assemblePluginPrompt` / `completeWithContext`；移除 legacy prepareContext / complete-draft |
...
| 2026-07-08 | 插件 API 链简化为三层 `pluginSettings`（[`DOC/devNotes/43`](43-plugin-api-binding-audit-checklist.md)） |
| 2026-07-07 | **宿主去特化**：`host.plugin.runAction` + manifest `serverActions`；`draft.kind` 改为 opaque `string`；settings schema widget 文档 |
| 2026-07-07 | §10 通知中心改为 **localStorage**（`DOC/devNotes/40`）；权限：`lorebook.read` 仅 lore 块时要求 |
