...
> **状态**：✅ **全部落地（M1–M5 · 2026-07-17 关闭）**——M5：引用扫描/强删清引用 · 多文件上传 · 视频预览（2026-07-15）；M4 独立文档 RAG 见 [`DOC/devNotes/46`](46-document-rag.md)（2026-07-17）；Markdown 导入（front matter 剥离）同日收尾。  
> **定案日期**：2026-06-08（M2 细节 2026-07-14；M3/M5 2026-07-15；M4/Markdown 2026-07-17）  
> **关联**：`DOC/devNotes/03` §15 宏、§17 文件库、`shared/file-media-token.ts`、`web/src/utils/authenticated-media-url.ts`；里程碑见 [`DOC/devNotes/04`](04-TODO.md) §已归档。
...
1. **用户级统一文件库**：上传与管理图片、文档、音频、视频；独立 UI（**不是**世界书编辑器）。**界面更名（2026-07-17）**：UI 文案统一称「资产库」（en: Asset library），与知识库共用 `LibraryHubView` 单一模态、tab 切换（主导航仅一个「文件」菜单项；`?panel=files` / `?panel=knowledge` 直达对应 tab）；后端概念与 API 路径仍为 files。
...
定案见 **[`DOC/devNotes/46-document-rag.md`](46-document-rag.md)**（M4）。

- **入口**：文件库上传文档 → 加入知识库（命名集合 `fileIds[]`）→ 切片 + embedding → **独立 Lance 表** `doc_chunks`。
- **明确不做**：文档管线 **不**替代世界书；绑定表与 RAG 索引分离；首版无 PDF。
- **索引策略**：FTS + scalar；知识库行数 ≥10k 时 IVF_PQ ANN（`DOC/devNotes/46` §3 / `DOC/devNotes/03` §14.4.2）。
...
- **M5 ✅**：`/files` 删除前拉引用列表；有引用则确认对话框 → 强删并清除绑定/对话媒体字段；多文件上传；网格/详情视频 `preload=metadata`；「修复」弹窗可粘贴破图 `/api/m/…` URL 或 8 hex ID 重建。
...
| **M4** ✅ 2026-07-17 | 文档切片 + 独立 Lance + 知识库 — 见 [`DOC/devNotes/46`](46-document-rag.md) | M1、embedding |
...
3. ~~知识库实体形态（M4）~~ → **已定**：命名集合，见 [`DOC/devNotes/46`](46-document-rag.md) §2。
4. ~~PDF 切片器选型（M4）~~ → **首版不支持**；后置选型（`DOC/devNotes/46` §4）。
5. 单用户文件配额（内网默认可不设）。
...
- ✅ 已更新 `DOC/devNotes/03` §17（M2/M3/M5/M4 小节）；`DOC/devNotes/04` 整节移入 **§已归档**（2026-07-17）。
