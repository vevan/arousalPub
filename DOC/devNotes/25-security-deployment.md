...
> **状态（2026-06）**：**已实现**（§2–§10；messages 分页 UI 见 `DOC/devNotes/15` ✅）。  
> **读者**：部署者、后续改 auth / 出站 / 插件的开发者。  
> **关联**：`DOC/devNotes/17`（运维台 loopback）、**§15 API Key**、`DOC/devNotes/24`（审计与 `customParams`）、Codex 2026-06 安全审计。
...
| `allowPublicRegister` | `true` | `false` 时 `POST /api/auth/register` 返回 `403`；开户改走 `POST /api/admin/users`（`DOC/devNotes/17`）。 |
...
| 访问方式 | 源 IP | whitelist | setup（§4.1） | admin（DOC/devNotes/17） |
|----------|-------|-----------|----------------|-----------------|
...
**反代**：默认**不信任** `X-Forwarded-For`（与 `DOC/devNotes/17` 一致）。仅在明确配置 `trustProxy` 后才用代理头解析客户端 IP（后续扩展）。
...
审计（`DOC/devNotes/24`）：记录**最终**出站 payload 或与其一致的摘要。
...
| 项 | 说明 |
|----|------|
| messages 默认分页 | `DOC/devNotes/15` S2–S4；非安全缺陷 |
...
| 2026-06-10 | §15：合并原 `DOC/devNotes/13`/`DOC/devNotes/16` API Key 定案摘要 |
...
- **轮换**：运维台 `DOC/devNotes/17` → `rotate-data-key`（维护模式 + 进度）。

### 15.3 与备份

全量冷备 zip 含加密 key 文件；恢复后 DEK 须一致。见 `data/README.md`、`DOC/devNotes/03` §8.8。
