...
> **实现**：`server/src/prompt-macros/`（**CST** 为唯一运行时引擎；路线图见 **`DOC/devNotes/29-cst-macros-compatibility.md`**）；可行性分级见 **`DOC/devNotes/14-st-macros-porting.md`**；运行时原则见 **`DOC/devNotes/03-实现细节.md` §15**。
...
- ST **`::` 多参**、块 `{{if}}` / scoped、`{{.var op …}}` 简写运算符均由 **CST** 解析（见 **`DOC/devNotes/29`**）。
...
| `{{notChar}}` | ✅ | 非群聊：除首绑卡外；群聊：除当前 speaker（`DOC/devNotes/35` §5） |
| `{{charAuthorsNote}}` | ⏳ | **`DOC/devNotes/28` Phase 2** |
...
| `{{original}}` `{{notChar}}`（群聊语义） | ✅ | 群聊下 `notChar` = 除当前 speaker；见 `DOC/devNotes/35` |
| `{{isMobile}}` | ⏳ | 可选 |
...
| Variable Shorthand **Operators**（`=` `+=` `++` `==` 标签等） | ✅ | D2.5；见 [DOC/devNotes/29](29-cst-macros-compatibility.md) §4.1 |
...
| ST 宏 / 能力 | 标记 | 原因 |
|--------------|------|------|
| `{{group}}` `{{groupNotMuted}}` `{{charIfNotGroup}}` | ✅ | 群聊定案见 [`DOC/devNotes/35`](35-group-chat.md) |
...
| 可行性备忘 | `DOC/devNotes/14-st-macros-porting.md` |
