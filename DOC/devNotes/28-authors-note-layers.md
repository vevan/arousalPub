...
> **定案日期**：2026-06-10  
> **关联**：`DOC/devNotes/03` §15.8（会话作者注已实现）；`DOC/devNotes/26`（ST 宏对照）；`server/src/authors-note-settings.ts`、`user-preferences-file.ts`。
...
authorsNote（会话）
  ...
  ▼
模型上下文（按 injectionDepth / role 插入 message 栈；深度锚点为最后一条 user 消息，见 `DOC/devNotes/03` §6.6）
