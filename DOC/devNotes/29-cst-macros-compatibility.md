...
> **ST 全量对照**：仍以 **`DOC/devNotes/26-st-macros-compatibility.md`** 为准（描述 legacy 已实现的 ST 兼容面）；本文只回答 **CST 相对 legacy 差什么**。
...
| 行为 | 说明 |
|------|------|
| 运行时 | **仅 CST**；`renderPromptMacros` → `renderPromptMacrosCst` |
| 未知 `{{…}}` | `[name UNSUPPORTED]` |
| 未闭合 `{{` | `[UNSUPPORTED]` |
...
实现：**`macro-shorthand-op.ts`**（解析 + 求值）；CST **`walker.ts`**（平衡 `}}`）。
...
| 能力 | 状态 | 备注 |
|------|------|------|
| `{{if}}` / `{{else}}` / `{{/if}}` | ✅ | `evaluateStCondition` |
| `{{#if}}`（ST 实验） | ✅ | Parser 剥 `#` 后与 `if` 同路径 |
| scoped `{{setvar}}`…`{{/setvar}}` | ✅ | `setvar` / `setglobalvar` / `reverse` / `trim` |
| `{{.name}}` / `{{$name}}` Get 简写 | ✅ | 无空格形态 → getvar / getglobalvar |
| `{{.var op …}}` 简写运算符 | ✅ | 见 §4.1 |
| `{{//}}` 注释 | ✅ | |
| `\{\{` / `\}\}` 转义 | ✅ | |
| 参数内嵌套 `{{setvar::k::{{char}}}}` | ✅ | |
| 宏标志 `!` `?` | ❌ | 未做 |
| 宏标志 `#`（scoped / if 保留空白） | ✅ | `#setvar` 等保留 body 首尾空白 |
| `addvar` | ✅ | 追加会话变量；`::` 值保留尾部换行 |
| 比较条件 `{{.x == y}}` / `!=` | ✅ | `macro-expr` + `evaluateStCondition` |
| 无参 `{{trim}}` | ✅ | 去**已输出**尾部空白 |
...
| Legacy 角括号 | `<USER>` `<CHAR>` `<BOT>` | ❌ 已移除（2026-07-08）；请用 `{{user}}` / `{{char}}` |
...
## 8. 与 DOC/devNotes/26 的关系

```text
DOC/devNotes/26  … ST 宏在本地「产品能力」总表（以 legacy 为完整实现基准）
DOC/devNotes/29  … 选用 CST 时，在 26 基础上再砍掉/尚未迁移的一层
```

查阅顺序：

1. 该宏在 **DOC/devNotes/26** 是否已实现？若否，CST 也不会 magically 有。  
...
| ST 全量对照 | `DOC/devNotes/26-st-macros-compatibility.md` |
| 运行时原则 | `DOC/devNotes/03-实现细节.md` §15 |
