...
> **关联**：`DOC/devNotes/03` §11.3、`DOC/devNotes/18` §3.13.1、`DOC/devNotes/30` 迹录侧栏。
...
- **勿在** `<router-view>` 或 `v-main` 上挂 `d-flex flex-column` 合并到子路由（见 `DOC/devNotes/03` §11.2 注释）
...
| `DOC/devNotes/03` §11.3 | 侧栏描述更新 |
| `DOC/devNotes/18` §3.13.1 | panel API 表更新 |
| `DOC/devNotes/30` | 侧栏占位从 drawer 改为 leftRail |

### 6.3 窄屏（**`DOC/devNotes/33`** · Phase 1 已落地）
...
- 仍待做：composer 键盘、`safe-area`（见 `DOC/devNotes/33`）
