...
> **定案日期**：2026-07-02  
> **关联**：`DOC/devNotes/04` §迁移、`DOC/devNotes/26`（ST 宏）、`DOC/devNotes/27`（角色卡内嵌世界书）、`DOC/devNotes/03` §6（chunk / turn）、§13（资料库）、§15（提示词预设）；样本 `.tmp/安娜 - 日记.jsonl`、`.tmp/希斯.json`（仅分析用，不入库）。
...
各库页面已有导入，但 **SillyTavern 迁移** 分散、无统一入口；`DOC/devNotes/04` P2「迁移」需产品化入口。
...
| 4 | **优先级 P2** | 与 `DOC/devNotes/04`「迁移」同级；可先落地 Tab 壳 + 预设跳转，再补聊天/世界书 API |
...
目标结构：`TurnRecord`（`server/src/chat-storage.ts`）— `turnId`、`turnOrdinal`、`send`、`segments[]` / `receives[]`、`activeReceiveIndex` 等；单 bot 导入时 **单 segment**，`speakerCharacterId = characterIds[0]`（对齐群聊 G0 迁移，`DOC/devNotes/35`）。
