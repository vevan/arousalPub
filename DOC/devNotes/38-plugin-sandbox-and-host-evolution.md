...
> **关联**：`DOC/devNotes/03` §1.3 插件 API 隔离 · `DOC/devNotes/04` P2 · `DOC/devNotes/09` · `DOC/devNotes/18` §4.1 · `DOC/devNotes/25` §15 · `server/src/plugin-system/loader.ts` · `server/src/plugin-host.ts`
...
| 项 | 实现 |
|----|------|
| 浏览器插件不持明文密钥 | `runPluginComplete` / `POST …/complete` 在宿主内解析 `apiConfigId` → credentials（`DOC/devNotes/03` §1.3） |
...
**`position.injectionOrder` 与 ST 一致**（`DOC/devNotes/03` §6.6 · preset `injectionOrder`）：**数值小 = 更靠近 user（post-user 区顶部）；数值大 = 更靠近栈底（生成前尾部）**。省略时默认 **100**（与 ST 导入 / 提示词 UI 一致）。**勿**与 preset 列表字段 `PromptEntry.order`（组内顺序）混淆；**勿**与插件 registry `order`（hook 调用顺序）混淆。
...
| **对话**插件 Tab | `conversationSettingsSchema` 的 `type: "apiPreset"` → **`pluginSettings[pluginId].apiConfigId`**（见 [`DOC/devNotes/43`](43-plugin-api-binding-audit-checklist.md) §1.1） |
...
解析链（`plugin-api-resolve.ts` · [`DOC/devNotes/43`](43-plugin-api-binding-audit-checklist.md) §1.1）：
...
**交叉项 checklist**：[`DOC/devNotes/43`](43-plugin-api-binding-audit-checklist.md)。
...
- 插件 capabilities 跨插件 RPC（见 `DOC/devNotes/09` §8.7）

---

## 8. 插件二次 LLM 上下文与拼 prompt

> **详案**：[`DOC/devNotes/39-plugin-context-and-prompt-assembly.md`](39-plugin-context-and-prompt-assembly.md)  
> **状态**：**Phase 1–3 已落地**（2026-07）；Sandbox Phase A 的前置依赖已满足。

与 §3 chat 注入**不同管线**；Worker 沙箱化前应避免 IPC 传递整段自拼 messages（已由 DOC/devNotes/39 解决）。
...
| 2026-07-07 | §8：链至 `DOC/devNotes/39`；标注 DOC/devNotes/39 前置已落地 |
...
| 2026-07-08 | §5：per-plugin API UI 定案 → 仅插件 Tab（链 [`DOC/devNotes/43`](43-plugin-api-binding-audit-checklist.md) §1.1） |
