...
trace-keeper Separate 已迁 **`completeWithContext`**（`TRACE_KEEPER_SEPARATE_LAYOUT` + `stripBlockTagsOnToTurn`）；Together 仍走 `afterAssemblePrompts`（`DOC/devNotes/38` chat 注入管线）。

### 1.2 目标（仍有效）

**声明式**：插件说明「要什么块 + 怎么排进 messages」；宿主读盘、拼 prompt、出站。与 `DOC/devNotes/38` §3（`/api/chat` outgoing 注入）**正交**。
...
```text
/api/chat 主对话
  assemble → trim → regex → afterAssemblePrompts（DOC/devNotes/38）
  → upstream
...
> Web 侧：`prepareContextBlocks` / `assemblePluginPrompt` / `completeWithContext` **均 proxy 至 Server**（`DOC/devNotes/18`）。  
...
### Phase 1 — 抽块 ✅（P0 · 与 [`DOC/devNotes/04`](04-TODO.md) 对齐）
...
| `DOC/devNotes/12` §5 | Historian 摘要上下文行为 |
| `DOC/devNotes/18` §3.8 | Web 宿主 API |
...
| 2026-07-07 | §6 分期优先级与 **`DOC/devNotes/04` P0** 对齐；合并 main 通知中心 localStorage 文档 |
