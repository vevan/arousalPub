...
> **状态**：**✅ 已完成并归档**（2026-07-09 · NC-V / NC-F1.V 签核 · [`DOC/devNotes/45`](45-notification-center-core-migration.md) §6.10）  
> **目标**：**所有用户可见瞬时消息**均由 **通知中心统一发出**；业务组件禁止自建 snackbar。浮层用 `v-snackbar-queue`；列表落盘规则见 `[DOC/devNotes/40](40-notification-center.md)` §3.1。  
> **前置**：NC1–NC5 已落地（store · bell · 插件 `host.ui.notify` · `level`）。  
> **关联**：`[DOC/devNotes/40](40-notification-center.md)` · `[DOC/devNotes/04](04-TODO.md)` §NC-F1 · `web/src/utils/core-notify.ts`
...
1. **Store 重构** — `notify` · `snackbarQueue` · `enqueueSnackbar` · `dismissSnackbar(id, reason)`（§3.1 `DOC/devNotes/40`）
...
### 3.4 列表与浮层行为（`[DOC/devNotes/40](40-notification-center.md)` §3.1）
...
与 `[DOC/devNotes/04](04-TODO.md)` 对齐，建议顺序：
...
> **语义速查**（`[DOC/devNotes/40](40-notification-center.md)` §3.1）：`notify` 默认 → 浮层 + **立即**入铃铛未读；**×** → 删列表；**超时** → 仅关浮层、列表保留；**业务钮** → 执行 action + 删列表。
...
- 将面板内 `v-alert` 迁入通知中心
- Server 推送 / SSE `notifications[]`（**延后** · NC-F2 · 见 `DOC/devNotes/40`）
...
| 文件                                          | 变更                                                    |
| ------------------------------------------- | ----------------------------------------------------- |
| `[DOC/devNotes/40](40-notification-center.md)`       | NC-F1.0–F1.5 已实现；代码索引更新                               |
| `[DOC/devNotes/04](04-TODO.md)`                      | NC-F1 子任务勾选                                           |
| `[DOC/devNotes/18](18-plugin-host-developer-api.md)` | 宿主 `coreNotify` 与插件 `notify` 共用存储                     |
...
| 2026-07-09 | 对齐 `DOC/devNotes/40`：`v-snackbar-queue` · 图标关闭 · 手动 × 删列表 · 超时保留                            |
