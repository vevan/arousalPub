...
> **2026-06-10 整理**：已删除完成态专项文档（i18n 迁移、API Key 定案、性能审计全文），内容并入 `DOC/devNotes/25`、`DOC/devNotes/03` §14、`DOC/devNotes/README`。  
> **权威顺序**：代码 > `DOC/devNotes/03` > 专题 > `DOC/devNotes/04`（待办）。
...
| [`20`](20-user-file-library.md) | ✅ **已归档**（2026-07-17） | 用户文件库 M1–M5 / 角色绑定宏 / 对话背景·BGM / 引用检查·批量导入 |
| [`46`](46-document-rag.md) | ✅ **已归档**（2026-07-17） | **独立文档 RAG（M4）**：知识库集合、切片、Lance、`boundKnowledge`、Markdown front matter 剥离 |
...
| [`32`](32-plugin-user-settings-cache.md) | ✅ | 全局 `getUserSettings` 缓存 + 订阅（对齐 `DOC/devNotes/21` 会话 store · `a7ca4ea`） |
...
| [`47`](47-graphify-optimization-backlog.md) | ✅ **已落地**（`Graphify`） | **Graphify 图谱审计**：server / web / plugins 全项；见 [`04`](04-TODO.md) §P0 |
...
| `DOC/devNotes/04` 对话分支 S1–S5 实现清单 | 2026-06-18 完成 → [`23`](23-conversation-branches.md) |
