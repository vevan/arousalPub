...
**Hybrid 全文检索分词**：全局默认可用中文 ngram / English / 中文 jieba / Lindera / **ICU** 等；**单场对话、单本资料库、单个知识库**可跟随全局或独立指定。
**改过分词方式后，仅重建受影响的索引**（跟随全局的资产；独立覆盖的资产不受全局变更影响）。远期记忆会话通常需手动点「重建记忆索引」。
选用 **Lindera** 时：可在设置里在线下载官方 v3.0.7 词典包，也可「从本地导入」任一受支持的官方 ZIP；系统会按 SHA-256 自动识别并选中对应规格，其他 major 会被拒绝。
**ICU** 无需词典，适合多语混排保底；中文主场景更推荐 **jieba**。
