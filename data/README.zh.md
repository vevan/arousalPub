...
应用运行时数据根目录，默认位于仓库根下的 `data/`（可在 `config.yaml` 的 `dataDir` 或环境变量 `DATA_DIR` / `AROUSAL_DATA_DIR` 中覆盖）。

请**不要**把真实用户数据提交进 Git。除本说明外，`data/` 下内容已被 `.gitignore` 忽略。
...
| `avatar.png` | 用户头像 |
| `chats/` | 对话会话与消息（`index.json` 可含 **`backgroundImageFileId`** / **`bgmFileId`**，见 `DOC/devNotes/20` M3）。分支在会话子目录；开启会话 debug 审计时可有 **`chat-audit.json`**（`DOC/devNotes/24`） |
| `prompts/` | 提示词预设（`index.json` + 各 `preset-*.json`） |
| `characters/` | 角色卡主存 **`{id}.png`**（`id` 为 8 位 hex）；宿主元数据 **`index.json`**：`userCardList`、**`imageFilesByCharacterId`**（见 `DOC/devNotes/03` §6.7 / §12） |
| `lorebooks/` | 资料库（`index.json` + 各书 JSON） |
...
| `user-preferences.json` | 全局偏好（含 embedding **`apiKeyEnc`**、**`hybridFts`**、默认作者注、预算裁切等，见 `DOC/devNotes/03` §14.4.3） |
| `memory/` | Lance 远期记忆等索引（**派生**，可重建；Syncthing 建议忽略，见 `DOC/devNotes/03` §14.5）— 对话在 `memory/conversations/`，知识库在 `memory/knowledge/` |
| `hybrid-fts/` | Hybrid BM25 分词资源（如 `zh-jieba/{variant}/…`；见 `DOC/devNotes/03` §14.4.3） |
| `regex-rules.json` | 原生正则规则（用户级；**无**会话级副本，见 **`DOC/devNotes/24`** §2.1、§6） |

新用户注册时会一次性写入英文 **种子**内容（默认 API 预设、提示词预设、资料库、正则规则）；启动过程不会反复补种。
