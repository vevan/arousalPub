...
async function exportConversation(
  ...
  if (maxOrd < 0 || rangeFrom > rangeTo) {
    host.ui.notify(host.t(k('notifyEmpty')), undefined, { level: 'info' })
    return
  }
  ...
  try {
    ...
    host.ui.notify(host.t(k('notifyDone')), undefined, { level: 'success' })
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e)
    host.ui.notify(`${host.t(k('notifyFailed'))}: ${msg}`, undefined, { level: 'error' })
  } finally {
