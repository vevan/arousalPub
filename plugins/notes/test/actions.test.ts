...
describe('parseNotesStore', () => {
  ...
  it('strips forged panel attrs and drops invalid entries on read', () => {
    const raw = JSON.stringify({
      version: 1,
      notes: [
        {
          id: 'a',
          title: 't data-tk-action="x"',
          body: '<b data-tk-field="y">ok</b>',
          createdAt: 1,
          updatedAt: 2,
        },
    ...
    const store = parseNotesStore(raw)
    assert.equal(store.notes.length, 1)
    assert.equal(store.notes[0]!.title.includes('data-tk'), false)
    assert.equal(store.notes[0]!.body.includes('data-tk'), false)
    assert.equal(store.notes[0]!.body.includes('<b'), true)
  })
})

describe('stripDataPluginAttrs', () => {
  it('removes data-tk-* attributes from pasted HTML', () => {
    const raw =
      'hello <button data-tk-action="delete:x" data-tk-field="y">x</button>'
    const out = stripDataPluginAttrs(raw)
    assert.equal(out.includes('data-tk-'), false)
...
describe('saveNote', () => {
  ...
  it('strips forged panel attrs on save', async () => {
    ...
    const { note } = await saveNote(
      {
        ...
        note: {
          title: 'x',
          body: '<span data-tk-action="delete:1">hi</span>',
        },
      },
      data,
    )
    assert.equal(note.body.includes('data-tk'), false)
