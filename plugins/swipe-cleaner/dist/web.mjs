// plugins/swipe-cleaner/src/web/index.ts
var PLUGIN_ID = "swipe-cleaner";
var BATCH_MAX = 50;
...
function getSegmentReceives(turn, segmentIndex) {
  ...
  const idx = typeof segmentIndex === "number" && Number.isFinite(segmentIndex) ? segmentIndex : getActiveSegmentIndex(turn);
...
function conversationHasSwipes(host) {
  ...
  return turns.some(
    (t) => (t.segments ?? []).some((seg) => (seg.receives?.length ?? 0) > 1)
  );
}
...
async function patchWithCheck(ctx, dtos) {
...
function register(host) {
  ...
  host.registerSlotButton("assistant-turn-footer", {
    ...
    when: (ctx) => !!ctx.turn && segmentSwipeCount(ctx.turn, ctx.segmentIndex) > 1,
    ...
    onClick: async (ctx) => {
      ...
      try {
        ...
        host.ui.notify(host.t(k("notifyTurnDone")), void 0, { level: "success" });
      } catch (e) {
        console.warn("[swipe-cleaner] turn clean failed", e);
        host.ui.notify(host.t(k("notifyFailed")), void 0, { level: "error" });
      }
    }
  });
  host.registerSlotButton("composer-toolbar", {
    ...
    onClick: async () => {
      ...
      try {
        ...
        host.ui.notify(host.t(k("notifyAllDone")), void 0, { level: "success" });
      } catch (e) {
