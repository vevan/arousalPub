...
function findTracePayloadInTurnPlugins(plugins, epoch) {
  ...
  for (let i = plugins.length - 1; i >= 0; i -= 1) {
    ...
    return {
      state,
      epoch: entryEpoch,
      receiveId: typeof payload.receiveId === "string" ? payload.receiveId : void 0
    };
  }
  return null;
}
...
function buildTrackerSystemPrompt(bundle, liveState) {
  ...
  const liveJson = liveState ? JSON.stringify(liveState, null, 2) : JSON.stringify(bundle.sampleState, null, 2);
  return [
    prefix,
...
async function afterAssemblePrompts(ctx, api) {
  ...
  if (isGuidanceGenerateRound(ctx.plugins ?? void 0)) return ctx.messages;
