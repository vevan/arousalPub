...
var SeparateRegenerateError = class extends Error {
  code;
  detail;
  debug;
...
var AUDIT_DEBUG_STORAGE_KEY = "arousal-chat-write-prompt-snapshot";
function readStoredAuditDebugPreference() {
  try {
...
function logSeparateDebugToConsole(debug) {
  const status = debug.code === "ok" ? "\u6210\u529F" : debug.code ? `\u5931\u8D25 (${debug.code})` : "\u5B8C\u6210";
  console.group(`[trace-keeper] Separate \u8865\u751F\u6210 debug \xB7 ${status}`);
  ...
  if (debug.upstreamRawBody?.trim() && debug.upstreamRawBody.trim() !== debug.assistantContent?.trim()) {
    console.log("raw body:", debug.upstreamRawBody);
  }
  if (debug.upstreamPayload !== void 0) {
    console.log("upstream payload:", debug.upstreamPayload);
  }
  console.groupEnd();
  console.groupEnd();
}
function logSeparateDebugIfPresent(debug) {
  if (!debug) return;
