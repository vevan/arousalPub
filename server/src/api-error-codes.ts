/** API 可本地化错误码（前端 api.errors.*） */
export const ApiErrorCodes = {
  active_preset_id_mismatch: 'active_preset_id_mismatch',
  active_receive_index_must_be_integer: 'active_receive_index_must_be_integer',
  admin_cannot_delete_seed: 'admin_cannot_delete_seed',
  admin_localhost_only: 'admin_localhost_only',
  admin_rotate_in_progress: 'admin_rotate_in_progress',
  admin_rotate_invalid_key: 'admin_rotate_invalid_key',
  admin_rotate_same_key: 'admin_rotate_same_key',
  admin_seed_user_required: 'admin_seed_user_required',
  api_credential_not_configured: 'api_credential_not_configured',
  api_key_not_found: 'api_key_not_found',
  api_key_in_use: 'api_key_in_use',
  api_key_reveal_failed: 'api_key_reveal_failed',
  api_preset_in_use: 'api_preset_in_use',
  api_preset_last_one: 'api_preset_last_one',
  api_preset_not_found: 'api_preset_not_found',
  api_preset_test_failed: 'api_preset_test_failed',
  api_preset_chat_test_failed: 'api_preset_chat_test_failed',
  api_keys_read_failed: 'api_keys_read_failed',
  api_keys_validation_failed: 'api_keys_validation_failed',
  api_keys_write_failed: 'api_keys_write_failed',
  api_key_reveal_wrong_password: 'api_key_reveal_wrong_password',
  append_turn_failed: 'append_turn_failed',
  assistant_content_empty_no_persist: 'assistant_content_empty_no_persist',
  auth_required: 'auth_required',
  auth_rate_limited: 'auth_rate_limited',
  auth_session_expired: 'auth_session_expired',
  client_ip_not_allowed: 'client_ip_not_allowed',
  backup_in_progress: 'backup_in_progress',
  avatar_must_be_png: 'avatar_must_be_png',
  avatar_not_found: 'avatar_not_found',
  card_body_invalid: 'card_body_invalid',
  character_create_failed: 'character_create_failed',
  character_ids_must_be_string_array: 'character_ids_must_be_string_array',
  character_import_failed: 'character_import_failed',
  character_import_png_failed: 'character_import_png_failed',
  character_not_found: 'character_not_found',
  character_not_found_or_no_png: 'character_not_found_or_no_png',
  character_update_failed: 'character_update_failed',
  characters_read_failed: 'characters_read_failed',
  image_files_too_many: 'image_files_too_many',
  image_files_name_conflict: 'image_files_name_conflict',
  image_files_body_invalid: 'image_files_body_invalid',
  chat_list_read_failed: 'chat_list_read_failed',
  conversation_already_exists: 'conversation_already_exists',
  conversation_create_failed: 'conversation_create_failed',
  conversation_delete_error: 'conversation_delete_error',
  conversation_delete_failed: 'conversation_delete_failed',
  conversation_no_tail_chunk: 'conversation_no_tail_chunk',
  conversation_not_found: 'conversation_not_found',
  conversation_api_preset_chat_invalid: 'conversation_api_preset_chat_invalid',
  conversation_api_preset_forbidden_field: 'conversation_api_preset_forbidden_field',
  conversation_api_preset_unknown_field: 'conversation_api_preset_unknown_field',
  conversation_api_preset_id_invalid: 'conversation_api_preset_id_invalid',
  conversation_api_preset_model_invalid: 'conversation_api_preset_model_invalid',
  conversation_api_preset_stream_boolean: 'conversation_api_preset_stream_boolean',
  conversation_api_preset_dry_breakers_array: 'conversation_api_preset_dry_breakers_array',
  conversation_api_preset_reasoning_boolean: 'conversation_api_preset_reasoning_boolean',
  conversation_api_preset_custom_params_string: 'conversation_api_preset_custom_params_string',
  conversation_embedding_api_invalid: 'conversation_embedding_api_invalid',
  conversation_embedding_api_forbidden_field: 'conversation_embedding_api_forbidden_field',
  delete_account_failed: 'delete_account_failed',
  embedding_api_api_key_id_invalid: 'embedding_api_api_key_id_invalid',
  embedding_api_api_key_string: 'embedding_api_api_key_string',
  embedding_api_base_url_string: 'embedding_api_base_url_string',
  embedding_api_embedding_dimensions_invalid: 'embedding_api_embedding_dimensions_invalid',
  embedding_api_embedding_model_string: 'embedding_api_embedding_model_string',
  embedding_provider_invalid: 'embedding_provider_invalid',
  embedding_test_failed: 'embedding_test_failed',
  embedding_prepare_rate_limited: 'embedding_prepare_rate_limited',
  builtin_embedding_input_too_large: 'builtin_embedding_input_too_large',
  first_turn_already_saved: 'first_turn_already_saved',
  first_turn_persist_maybe_exists: 'first_turn_persist_maybe_exists',
  first_turn_write_failed: 'first_turn_write_failed',
  file_not_found: 'file_not_found',
  file_not_document: 'file_not_document',
  file_kind_mismatch: 'file_kind_mismatch',
  file_mime_not_allowed: 'file_mime_not_allowed',
  file_invalid_id: 'file_invalid_id',
  file_id_taken: 'file_id_taken',
  file_name_invalid: 'file_name_invalid',
  file_tags_invalid: 'file_tags_invalid',
  file_create_failed: 'file_create_failed',
  file_update_failed: 'file_update_failed',
  file_delete_failed: 'file_delete_failed',
  file_in_use: 'file_in_use',
  files_read_failed: 'files_read_failed',
  global_embedding_api_requires_field: 'global_embedding_api_requires_field',
  global_history_requires_field: 'global_history_requires_field',
  global_lorebook_requires_field: 'global_lorebook_requires_field',
  global_memory_requires_field: 'global_memory_requires_field',
  global_chunk_requires_field: 'global_chunk_requires_field',
  global_default_authors_note_requires_field: 'global_default_authors_note_requires_field',
  default_authors_note_invalid: 'default_authors_note_invalid',
  default_authors_note_enabled_for_new_chats_boolean:
    'default_authors_note_enabled_for_new_chats_boolean',
  chunk_turns_per_file_number: 'chunk_turns_per_file_number',
  chunk_chain_broken: 'chunk_chain_broken',
  chunk_index_repair_failed: 'chunk_index_repair_failed',
  authors_note_content_string: 'authors_note_content_string',
  authors_note_enabled_boolean: 'authors_note_enabled_boolean',
  authors_note_injection_depth_number: 'authors_note_injection_depth_number',
  authors_note_invalid: 'authors_note_invalid',
  authors_note_requires_field: 'authors_note_requires_field',
  authors_note_role_invalid: 'authors_note_role_invalid',
  budget_trim_min_retain_invalid: 'budget_trim_min_retain_invalid',
  budget_trim_min_retain_number: 'budget_trim_min_retain_number',
  budget_trim_min_retain_requires_field: 'budget_trim_min_retain_requires_field',
  budget_trim_settings_invalid: 'budget_trim_settings_invalid',
  budget_trim_settings_requires_field: 'budget_trim_settings_requires_field',
  budget_trim_trim_order_invalid: 'budget_trim_trim_order_invalid',
  branch_create_failed: 'branch_create_failed',
  branch_delete_failed: 'branch_delete_failed',
  branch_label_invalid: 'branch_label_invalid',
  branch_registry_broken: 'branch_registry_broken',
  conversation_chunks_unreadable: 'conversation_chunks_unreadable',
  branch_update_failed: 'branch_update_failed',
  branch_path_conflict: 'branch_path_conflict',
  branch_path_not_found: 'branch_path_not_found',
  global_budget_trim_requires_field: 'global_budget_trim_requires_field',
  fork_turn_not_found: 'fork_turn_not_found',
  fork_turn_not_on_active_path: 'fork_turn_not_on_active_path',
  fork_turn_has_branches: 'fork_turn_has_branches',
  turn_not_on_active_path: 'turn_not_on_active_path',
  chunk_read_failed: 'chunk_read_failed',
  chunk_write_failed: 'chunk_write_failed',
  history_limit_enabled_boolean: 'history_limit_enabled_boolean',
  history_max_turns_number: 'history_max_turns_number',
  history_settings_invalid: 'history_settings_invalid',
  history_settings_limit_enabled_boolean: 'history_settings_limit_enabled_boolean',
  history_settings_max_turns_number: 'history_settings_max_turns_number',
  history_settings_requires_field: 'history_settings_requires_field',
  invalid_conversation_id: 'invalid_conversation_id',
  invalid_credentials: 'invalid_credentials',
  invalid_id: 'invalid_id',
  invalid_plugin_id: 'invalid_plugin_id',
  invalid_request_body: 'invalid_request_body',
  invalid_turn_ordinal: 'invalid_turn_ordinal',
  group_continue_invalid: 'group_continue_invalid',
  group_continue_speaker_duplicate: 'group_continue_speaker_duplicate',
  invalid_user: 'invalid_user',
  kb_id_taken: 'kb_id_taken',
  knowledge_base_ids_must_be_string_array: 'knowledge_base_ids_must_be_string_array',
  knowledge_base_not_found: 'knowledge_base_not_found',
  knowledge_bases_read_failed: 'knowledge_bases_read_failed',
  knowledge_bases_write_failed: 'knowledge_bases_write_failed',
  knowledge_settings_invalid: 'knowledge_settings_invalid',
  lorebook_ids_must_be_string_array: 'lorebook_ids_must_be_string_array',
  lorebook_max_recursion_depth_number: 'lorebook_max_recursion_depth_number',
  lorebook_keyword_top_k_number: 'lorebook_keyword_top_k_number',
  lorebook_not_found: 'lorebook_not_found',
  lorebook_recursive_enabled_boolean: 'lorebook_recursive_enabled_boolean',
  lorebook_settings_invalid: 'lorebook_settings_invalid',
  lorebook_settings_max_recursion_depth_number: 'lorebook_settings_max_recursion_depth_number',
  lorebook_settings_keyword_top_k_number: 'lorebook_settings_keyword_top_k_number',
  lorebook_settings_recursive_enabled_boolean: 'lorebook_settings_recursive_enabled_boolean',
  lorebook_settings_requires_field: 'lorebook_settings_requires_field',
  lorebook_settings_vector_enabled_boolean: 'lorebook_settings_vector_enabled_boolean',
  lorebook_settings_vector_top_k_number: 'lorebook_settings_vector_top_k_number',
  lorebook_vector_enabled_boolean: 'lorebook_vector_enabled_boolean',
  lorebook_vector_top_k_number: 'lorebook_vector_top_k_number',
  lorebooks_bulk_put_payload_too_large: 'lorebooks_bulk_put_payload_too_large',
  lorebooks_bulk_put_rate_limited: 'lorebooks_bulk_put_rate_limited',
  lorebooks_read_failed: 'lorebooks_read_failed',
  lorebooks_validation_failed: 'lorebooks_validation_failed',
  lorebooks_write_failed: 'lorebooks_write_failed',
  memory_enabled_boolean: 'memory_enabled_boolean',
  memory_rebuild_failed: 'memory_rebuild_failed',
  context_recall_query_required: 'context_recall_query_required',
  context_recall_topk_invalid: 'context_recall_topk_invalid',
  context_recall_simulate_turn_invalid: 'context_recall_simulate_turn_invalid',
  memory_vector_index_corrupt: 'memory_vector_index_corrupt',
  memory_settings_invalid: 'memory_settings_invalid',
  hybrid_fts_settings_invalid: 'hybrid_fts_settings_invalid',
  hybrid_fts_dict_not_ready: 'hybrid_fts_dict_not_ready',
  hybrid_fts_dict_import_not_supported: 'hybrid_fts_dict_import_not_supported',
  hybrid_fts_dict_package_mismatch: 'hybrid_fts_dict_package_mismatch',
  hybrid_fts_dict_archive_invalid: 'hybrid_fts_dict_archive_invalid',
  hybrid_fts_dict_install_conflict: 'hybrid_fts_dict_install_conflict',
  hybrid_fts_dict_import_failed: 'hybrid_fts_dict_import_failed',
  memory_settings_memory_enabled_boolean: 'memory_settings_memory_enabled_boolean',
  memory_settings_memory_top_k_number: 'memory_settings_memory_top_k_number',
  memory_settings_requires_field: 'memory_settings_requires_field',
  memory_top_k_number: 'memory_top_k_number',
  memory_strip_plugin_blocks_boolean: 'memory_strip_plugin_blocks_boolean',
  memory_strip_ex_prefix_boolean: 'memory_strip_ex_prefix_boolean',
  memory_strip_block_tags_invalid: 'memory_strip_block_tags_invalid',
  memory_recall_fuse_last_assistant_boolean: 'memory_recall_fuse_last_assistant_boolean',
  memory_recall_user_weight_number: 'memory_recall_user_weight_number',
  maintenance_mode: 'maintenance_mode',
  messages_item_role_content: 'messages_item_role_content',
  messages_required_nonempty: 'messages_required_nonempty',
  missing_api_key: 'missing_api_key',
  missing_assistant_content: 'missing_assistant_content',
  missing_avatar_field: 'missing_avatar_field',
  missing_confirm_username: 'missing_confirm_username',
  missing_conversation_id: 'missing_conversation_id',
  missing_conversation_or_user_text: 'missing_conversation_or_user_text',
  missing_file_field: 'missing_file_field',
  missing_model: 'missing_model',
  missing_password_fields: 'missing_password_fields',
  missing_portrait_field: 'missing_portrait_field',
  missing_presets_array: 'missing_presets_array',
  missing_refresh_token: 'missing_refresh_token',
  missing_user_content: 'missing_user_content',
  missing_user_text: 'missing_user_text',
  missing_username_or_password: 'missing_username_or_password',
  models_list_failed: 'models_list_failed',
  multipart_payload_required: 'multipart_payload_required',
  name_required: 'name_required',
  opening_persist_failed: 'opening_persist_failed',
  opening_write_failed: 'opening_write_failed',
  password_change_failed: 'password_change_failed',
  patch_conversation_requires_field: 'patch_conversation_requires_field',
  payload_invalid_json: 'payload_invalid_json',
  persist_error: 'persist_error',
  persist_cancelled_by_user: 'persist_cancelled_by_user',
  persist_failed: 'persist_failed',
  setup_localhost_only: 'setup_localhost_only',
  png_image_required: 'png_image_required',
  portrait_upload_failed: 'portrait_upload_failed',
  preset_validation_failed: 'preset_validation_failed',
  public_register_disabled: 'public_register_disabled',
  prompt_preset_id_invalid: 'prompt_preset_id_invalid',
  prompt_preset_unresolved: 'prompt_preset_unresolved',
  plugin_registry_invalid: 'plugin_registry_invalid',
  plugin_registry_read_failed: 'plugin_registry_read_failed',
  plugin_not_found: 'plugin_not_found',
  plugin_disabled: 'plugin_disabled',
  plugin_permission_denied: 'plugin_permission_denied',
  plugin_complete_failed: 'plugin_complete_failed',
  plugin_complete_stream_not_supported: 'plugin_complete_stream_not_supported',
  plugin_complete_context_exceeded: 'plugin_complete_context_exceeded',
  plugin_complete_context_length_unconfigured:
    'plugin_complete_context_length_unconfigured',
  plugin_complete_draft_failed: 'plugin_complete_draft_failed',
  plugin_hook_not_supported: 'plugin_hook_not_supported',
  plugin_prepare_context_failed: 'plugin_prepare_context_failed',
  plugin_assemble_prompt_failed: 'plugin_assemble_prompt_failed',
  plugin_complete_with_context_failed: 'plugin_complete_with_context_failed',
  draft_kind_invalid: 'draft_kind_invalid',
  feature_binding_api_config_invalid: 'feature_binding_api_config_invalid',
  feature_binding_duplicate: 'feature_binding_duplicate',
  feature_binding_invalid: 'feature_binding_invalid',
  feature_binding_not_found: 'feature_binding_not_found',
  feature_binding_ref_invalid: 'feature_binding_ref_invalid',
  feature_binding_type_invalid: 'feature_binding_type_invalid',
  feature_bindings_empty: 'feature_bindings_empty',
  feature_bindings_invalid: 'feature_bindings_invalid',
  feature_bindings_read_failed: 'feature_bindings_read_failed',
  feature_bindings_write_failed: 'feature_bindings_write_failed',
  user_content_required: 'user_content_required',
  system_prompt_required: 'system_prompt_required',
  invalid_turn_range: 'invalid_turn_range',
  no_turns_in_range: 'no_turns_in_range',
  turn_range_too_large: 'turn_range_too_large',
  target_lorebook_required: 'target_lorebook_required',
  lorebook_id_required: 'lorebook_id_required',
  plugin_settings_invalid: 'plugin_settings_invalid',
  plugin_settings_import_invalid: 'plugin_settings_import_invalid',
  plugin_settings_plugin_mismatch: 'plugin_settings_plugin_mismatch',
  lorebook_entry_not_found: 'lorebook_entry_not_found',
  lorebook_entry_create_failed: 'lorebook_entry_create_failed',
  lorebook_entry_patch_failed: 'lorebook_entry_patch_failed',
  lorebook_order_invalid: 'lorebook_order_invalid',
  lorebook_entry_validation_failed: 'lorebook_entry_validation_failed',
  prompts_preview_failed: 'prompts_preview_failed',
  prompts_preset_not_found: 'prompts_preset_not_found',
  prompts_read_failed: 'prompts_read_failed',
  prompts_unavailable: 'prompts_unavailable',
  prompts_validation_failed: 'prompts_validation_failed',
  prompts_write_failed: 'prompts_write_failed',
  prompts_seed_put_rejected: 'prompts_seed_put_rejected',
  prompts_not_initialized: 'prompts_not_initialized',
  prompts_st_convert_failed: 'prompts_st_convert_failed',
  receives_content_required: 'receives_content_required',
  receives_item_id_content_required: 'receives_item_id_content_required',
  receives_item_invalid: 'receives_item_invalid',
  receives_required_nonempty: 'receives_required_nonempty',
  refresh_token_expired: 'refresh_token_expired',
  regenerate_turn_not_found: 'regenerate_turn_not_found',
  reindex_failed: 'reindex_failed',
  settings_read_failed: 'settings_read_failed',
  settings_write_failed: 'settings_write_failed',
  turn_chunk_not_found: 'turn_chunk_not_found',
  turn_delete_failed: 'turn_delete_failed',
  turn_delete_not_found: 'turn_delete_not_found',
  turn_update_failed: 'turn_update_failed',
  turns_batch_too_large: 'turns_batch_too_large',
  turns_batch_required: 'turns_batch_required',
  messages_range_incomplete: 'messages_range_incomplete',
  messages_range_invalid: 'messages_range_invalid',
  range_too_large: 'range_too_large',
  regex_apply_invalid_phase: 'regex_apply_invalid_phase',
  regex_apply_invalid_field: 'regex_apply_invalid_field',
  regex_apply_tail_ordinal_invalid: 'regex_apply_tail_ordinal_invalid',
  regex_apply_text_required: 'regex_apply_text_required',
  regex_rules_read_failed: 'regex_rules_read_failed',
  regex_rules_validation_failed: 'regex_rules_validation_failed',
  regex_rules_write_failed: 'regex_rules_write_failed',
  regex_batch_invalid_range: 'regex_batch_invalid_range',
  regex_exec_timeout: 'regex_exec_timeout',
  upstream_api_error: 'upstream_api_error',
  upstream_url_invalid: 'upstream_url_invalid',
  upstream_url_private_forbidden: 'upstream_url_private_forbidden',
  upstream_url_scheme_forbidden: 'upstream_url_scheme_forbidden',
  upstream_non_json: 'upstream_non_json',
  user_character_id_invalid: 'user_character_id_invalid',
  user_name_invalid: 'user_name_invalid',
  user_not_ready: 'user_not_ready',
  user_preferences_read_failed: 'user_preferences_read_failed',
  user_preferences_requires_section: 'user_preferences_requires_section',
  user_preferences_save_failed: 'user_preferences_save_failed',
  post_user_injection_order_invalid: 'post_user_injection_order_invalid',
  post_user_injection_order_requires_field: 'post_user_injection_order_requires_field',
  hybrid_fts_profile_invalid: 'hybrid_fts_profile_invalid',
  hybrid_fts_dict_variant_invalid: 'hybrid_fts_dict_variant_invalid',
  user_text_must_be_string: 'user_text_must_be_string',
  st_import_invalid_format: 'st_import_invalid_format',
  st_lorebook_import_failed: 'st_lorebook_import_failed',
  st_lorebook_too_many_entries: 'st_lorebook_too_many_entries',
  st_chat_import_failed: 'st_chat_import_failed',
  st_chat_conversation_not_empty: 'st_chat_conversation_not_empty',
  st_chat_bindings_required: 'st_chat_bindings_required',
  validation_failed: 'validation_failed',
} as const

export type ApiErrorCode = (typeof ApiErrorCodes)[keyof typeof ApiErrorCodes]

export {
  UserAccountErrorCodes,
  type UserAccountErrorCode,
} from './user-account-error.js'
