...
describe('normalizeBudgetTrimSettings', () => {
  it('uses defaults for empty input', () => {
    ...
    assert.deepEqual(s.trimOrder, ['lore', 'memory', 'history'])
    assert.deepEqual(s.minRetain, { lore: 1, memory: 1, history: 1 })
  })

  it('clamps minRetain', () => {
    const s = normalizeBudgetTrimSettings({
      minRetain: { lore: -1, memory: 99, history: 2 },
    })
...
describe('parseBudgetTrimSettingsPatch', () => {
  it('parses trimOrder patch', () => {
    const r = parseBudgetTrimSettingsPatch({
      trimOrder: ['history', 'memory', 'lore'],
    })
    assert.equal(r.ok, true)
    if (r.ok) {
      assert.deepEqual(r.patch.trimOrder, ['history', 'memory', 'lore'])
    }
  })
})
