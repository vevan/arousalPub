/** 与 SillyTavern 等常见习惯对齐的占位符替换（大小写不敏感） */

export interface PromptMacroContext {
  /** 对应 `{{user}}`；缺省由调用方给默认展示名 */
  userName: string
  /**
   * 按会话绑定顺序：下标 0 → `{{char}}` / `{{char1}}`，下标 1 → `{{char2}}`…
   */
...
/**
 * 替换 `{{user}}`、`{{char}}`、`{{char1}}`…`{{char9}}`（数字与 ST 多卡习惯一致）。
 * 无 `{{` 时原样返回，避免无谓分配。
 */
...
/** 已知宏展开后，仍残留的 `{{...}}` 替换为 `[UNSET]`（含未闭合的 `{{` 尾部） */
